...
function CustomSearch ({previousStep, id}) { 
  ...
  useEffect(() => {
    const postMessage = async () => {
      ...
      setLoading(true);
      const resPost = await axios.post('http://localhost:4000/messages', postParams);
      const statusCodePost = resPost.data.statusCode;

      if(statusCodePost === 200) {
        const messageGroupId = resPost.data.body.messageGroupId;
        const resGet = await axios.get(`http://localhost:4000/messages/${messageGroupId}`, postParams);
